...
function UserMenu() {
    ...
    const getSignature = async () => {
        ...
        console.log(158, new Date().toLocaleString('ja-JP', {
            timeZone: 'GMT',
            hour12: false,
        }).replace(/\//g, '-').replace(',', ''));
        ...
        socket.off('message').on('message', (data) => {
            ...
            if (data.connectingLink === 'terminal joined' && uniqueId === data.uniqueId) {
                console.log(155123);
                stopRingtone();
                startMeeting();
            }

            if (data.connectingLink === 'no active terminal' && uniqueId === data.uniqueId) {
                console.log(155123);
                stopRingtone();
                alert('no active terminal');
            }

            if (data.connectingLink === 'forcible disconnect' && uniqueId === data.uniqueId) {
                const now = new Date();
                ...
                stopRingtone();
                alert('通訳者 が応答できない状態です。しばらくお待ちいただき、もう一度、Call ボタンを押してください。');
            }
        });
    ...
    return (
        <div className="user-menu">
            ...
            <div className="user-call-row" >
                    <ButtonAtom
                        onClick={() => { playRingtone(); getSignature(); }}
                        label="Call"
                        width="50px"
                        disabled={isPlaying}
                        padding="16px"
                    />
                    <ButtonAtom
                        onClick={() => { stopRingtone(); emitDisconnectInfo(); }}
                        label="Terminate"
                        width="100px"
                        disabled={!isPlaying}
                        padding="16px"
